...
function render({focusTitle=false}={}){
 ...
 const r=model(),st=stages[s.stage],done=finished(),step=guide(),demoCircuit=s.tour===0&&s.stage===0?{...s,wires:SINGLE_WIRES,closed:true}:s,demoResult=s.tour===0&&s.stage===0?solveCircuit({...demoCircuit,secondResistance:demoCircuit.resistance}):r; if(done&&!completed.includes(s.stage))completed.push(s.stage);save();
