...
import {coachOverlay,coachControls,coachSteps} from './coach.js';
import {circuitView,graphView,icons} from './circuit.js';

const KEY='electric-flow-v1', app=document.querySelector('#app');
let s=initialStage(0), completed=[], stageStates={}, drawer=false, modal=null, history=[], saveAvailable=true, measureTimer,celebrationStage=null,celebrationTimer,home=true,pointerDrag=null,skipDragClick=false,demoSeen={},demoStep=0,demoSession=null,wiringDemoOpen=false,coachTimer;
function restoreState(raw) {
 ...
 if(Number.isInteger(raw.helpStep)&&raw.helpStep>=0&&raw.helpStep<=2)base.helpStep=raw.helpStep;
...
function pendingDemo(){if(home||modal||drawer)return null;if(s.stage===0&&wiringDemoOpen)return'wiring';if(s.stage===2&&s.tour&&s.range&&!demoSeen.current)return'current';if(s.stage===3&&!demoSeen.voltage)return'voltage';return null;}
function firstDemoStep(type){
 if(type==='wiring')return 0;
 if(type==='current')return recorded('single-before')&&!recorded('single-after')?1:0;
 if(type==='voltage'){const index=['lamp1','battery','wire'].findIndex(k=>!recorded(`single-V-${k}`));return Math.max(0,index);}
 return 0;
}
function markDemoSeen(type){if(!type)return;demoStep=0;if(type==='wiring')wiringDemoOpen=false;demoSeen[type]=true;saveDemoSeen();}
...
function symbolKey(){return `<div class="symbol-key" aria-label="式の文字の意味"><span><b>R</b> 抵抗器の抵抗 <small>Ω</small></span><span><b>V</b> かけた電圧 <small>V</small></span><span><b>I</b> 流れた電流 <small>A</small></span></div>`;}
function problemHelp(kind){
 const hints={
  graph:['表の 2.0 V の行は 0.20 A。4.0 V はその2倍です。','電流も2倍なので 0.20 × 2。横 4.0 V の点を見ます。'],
  resistance:['求める R は「抵抗器の抵抗」。電圧 V を電流 I で割ります。','この問題の V は 3.0 V、I は 0.15 A。3.0 ÷ 0.15 を計算します。'],
  connection:['電圧計は部品をはさむように2点につなぎます。','図の四角い抵抗器のすぐ左と右にある端子を探します。'],
  examPlot:['表の 3.0 V と同じ行の電流を読みます。','その行の電流は 0.30 A。横 3.0、縦 0.30 の点です。'],
  examResistance:['表の同じ行から V と I を取ります。R＝V÷I です。','1.0 V の行なら I は 0.10 A。1.0 ÷ 0.10 を計算します。'],
  examCurrent:['求めるのは I（電流）。電圧 V を抵抗 R で割ります。','前の問いで R は 10 Ω。4.0 ÷ 10 を計算します。'],
  examCondition:['電圧と電流の関係を調べたい実験です。','同じ部品のまま電圧だけを変えると、電流を比べられます。']
 }[kind];
 const answers={graph:'4.0 V では 0.40 A。横 4.0、縦 0.40 の点を選びます。',resistance:'R＝3.0÷0.15＝20 Ω。20 と入力します。',connection:'抵抗器の左 A と右 B を選びます。',examPlot:'横 3.0 V、縦 0.30 A の点を選びます。',examResistance:'R＝1.0÷0.10＝10 Ω。10 と入力します。',examCurrent:'I＝4.0÷10＝0.40 A。0.40 と入力します。',examCondition:'変えずにおくのは抵抗器です。'};
 return `<div class="problem-help">${s.helpStep<2?`<button class="text-button help-trigger" data-action="problem-hint" aria-expanded="${s.helpStep>0}">${s.helpStep?'次のヒントを見る':'わからないときはヒント'}</button>`:''}${s.helpStep?`<div class="help-steps" role="note">${hints.slice(0,s.helpStep).map((h,i)=>`<p><b>ヒント ${i+1}</b>${h}</p>`).join('')}</div>`:''}${s.helpStep>=2?(s.answerShown?`<p class="worked-answer"><b>見本</b>${answers[kind]}</p>`:'<button class="text-button answer-trigger" data-action="answer">解き方と答えを見る</button>'):''}</div>`;
}
...
function controls(r){
 const done=finished();
 switch(s.stage){
 case 0:return `<div class="panel-eyebrow">YOUR FIRST CIRCUIT</div><h2>小さな実験室へ。</h2><p class="panel-copy">3本つないだら、最後にスイッチを閉じる。<button class="text-button demo-link" data-action="replay-demo">見本</button></p>${!done&&!completed.includes(0)?`<div class="current-task"><span>今やること</span><p>${guide().text}</p></div>`:''}${checks([['電池 → 電球',s.wires.some(e=>sameEdge(e,['p','a']))],['電球 → スイッチ',s.wires.some(e=>sameEdge(e,['b','sr']))],['スイッチ → 電池',s.wires.some(e=>sameEdge(e,['sl','n']))],['スイッチを閉じる',s.closed]])}<div class="live-readings">${reading('電池',pretty(s.voltage,1),'V')}${reading('回路全体',pretty(r.current),'A')}</div>`;
 ...
 case 2:if(!s.tour)return `<div class="panel-eyebrow">MEASUREMENT BASICS</div><h2>はじめに、つなぎ方を知ろう。</h2><div class="basics-list"><p><b>電流計</b><span>流れる電流をはかる道具。導線の途中に入れます。電池の＋側を計器の＋端子へ。</span></p><p><b>電圧計</b><span>部品の左と右など、はかりたい部分の両端につなぎます。</span></p><p><b>測定範囲</b><span>最初は一番大きい 3 A から。mA は A の千分の一です。</span></p></div><button class="primary wide" data-action="begin-practice">わかったら、ためす ${icons.arrow}</button>`;
 return `<div class="panel-eyebrow">CURRENT METER</div><h2 class="meter-heading">電流計を途中へ。<button class="topic-help" data-action="meter-help" aria-label="電流計とは？">？</button></h2><p class="panel-copy">電流計は回路の途中に入れる。${s.range?'<button class="text-button demo-link" data-action="replay-demo">見本</button>':''}</p>${!s.range?`<div class="range-task"><div class="meter-focus attention"><span class="meter-icon">A</span><span>電流計</span></div><p class="mini-question">まず、どれからつかう？</p><div class="range-choices"><button data-range="3A">3 A</button><button data-range="300mA">300 mA</button><button data-range="30mA">30 mA</button></div></div>`:`<div class="drag-task"><button class="meter-drag attention" data-drag-meter draggable="true" aria-label="Aを押したまま電球の前の丸いプラスへ運ぶ"><span class="meter-icon">A</span><span>Aを回路へ運ぶ</span></button><p class="quiet-note">Aを押したまま、左の丸い「＋」へ。丸い「＋」をタップしても入れられます。</p></div>`}${checks([['電球の前',recorded('single-before')],['電球の後',recorded('single-after')]])}${recorded('single-before')&&recorded('single-after')?`<div class="mini-question">前と後の電流は？</div>${choice('current',['同じ','後のほうが小さい'])}`:''}`;
 case 3:return `<div class="panel-eyebrow">VOLTAGE METER</div><h2>電圧計を、両端につなごう。</h2><p class="panel-copy">はかりたい部分の両端を選ぶ。<button class="text-button demo-link" data-action="replay-demo">見本</button></p><div class="drag-task"><button class="meter-drag" data-drag-probe draggable="true" aria-label="Vの線を押したまま端子へ運ぶ"><span class="meter-icon">V</span><span>Vの線を端子へ運ぶ</span></button><p class="quiet-note">電球の左と右の端子を順にタップ。Vを1本ずつ運んでもつなげられます。</p></div>${checks([['電球の両端',recorded('single-V-lamp1')],['電池の両端',recorded('single-V-battery')],['つながった導線（0 V）',recorded('single-V-wire')]])}<div class="live-readings">${reading('測った電圧',pretty(measuredVoltage(r,s.probes)),'V')}</div>`;
 case 4:return `<div class="panel-eyebrow">TWO PATHS</div><h2>道が違うと、どうなる？</h2><p class="concept-note">直列は電流の道が1本、並列は2本に分かれます。電球 B を外して、もう一方がつくか見てみよう。</p>${topologyControls()}${checks([['直列でくらべる',has('series')&&has('removed-series')],['並列でくらべる',has('parallel')&&has('removed-parallel')]])}${has('removed-series')&&has('removed-parallel')?`<div class="mini-question">並列で、電球 Bを外すと？</div>${choice('branch',['もう一つは、つく','どちらも消える'])}`:''}<button class="secondary wide" data-action="remove">${s.removed?'電球 Bを戻す':'電球 Bを外す'}</button>${reading('回路全体',pretty(r.current),'A')}`;
 ...
 ${parallel&&['parallel-before','parallel-branch1','parallel-branch2'].every(recorded)?`<div class="mini-question">枝が 0.40 A と 0.20 A。全体は？</div><p class="concept-note">枝分かれ前の電流は、2本の枝の電流を足したものです。</p>${choice('sum',['0.20 A','0.40 A','0.60 A'])}`:''}`;}
 case 6:return `<div class="panel-eyebrow">FIND A PATTERN</div><h2>電圧だけを、変える。</h2><p class="concept-note">図の下の部品は抵抗器。今は同じ抵抗器のまま、電圧を変えて電流を調べます。</p><div class="voltage-control">${reading('電圧',pretty(s.voltage,1),'V')}<input aria-label="電圧" type="range" id="voltage" min="1" max="5" step="1" value="${s.voltage}"><div class="voltage-steps">${[1,2,3,4,5].map(v=>`<button data-voltage="${v}" class="${s.voltage===v?'selected':''}">${v}<small> V</small></button>`).join('')}</div></div>${reading('電流',pretty(r.current),'A')}${!has('measuring')?'<button class="secondary wide" data-action="measure-start">測定をはじめる</button>':'<p class="recording"><i></i> 値が落ち着くと、自動で記録</p>'}${checks([['1・2・3 Vを記録',[1,2,3].every(v=>s.samples.some(p=>p.v===v))]])}${s.samples.length>=3?`<div class="mini-question">4 Vの点は、どこ？</div><p class="concept-note">グラフは横が電圧、縦が電流。電圧が2倍なら、電流も2倍になるか見てみよう。</p>${problemHelp('graph')}`:''}`;
 case 7:return `<div class="panel-eyebrow">OHM’S LAW</div><h2>同じ電圧で、抵抗器を取り替える。</h2><p class="concept-note">図の下にある部品が抵抗器です。抵抗は電流の流れにくさで、R と書き、単位は Ω（オーム）です。</p>${reading('電圧','3.0','V')}<div class="resistance-picker">${[10,20,30].map(v=>`<button data-resistance="${v}" class="${s.resistance===v?'active':''}" aria-pressed="${s.resistance===v}">${v}<small> Ω</small></button>`).join('')}</div>${reading('電流',pretty(r.current),'A')}${checks([10,20,30].map(v=>[`${v} Ω`,has(`R${v}`)]))}${[10,20,30].every(v=>has(`R${v}`))?`<div class="formula"><span>求めるもの</span><strong>R ＝ V ÷ I</strong></div>${symbolKey()}<p class="worked-example">たとえば、3.0 V で 0.30 A なら、抵抗は 3.0 ÷ 0.30 ＝ 10 Ω。</p><label class="mini-question" for="resistance-answer">では、3.0 V で 0.15 A が流れる抵抗器の抵抗 R は何 Ω？</label>${problemHelp('resistance')}<form data-form="resistance"><div class="answer-input"><input id="resistance-answer" name="answer" inputmode="decimal" autocomplete="off" aria-label="抵抗の答え" required><span>Ω</span><button aria-label="抵抗の答えを確かめる">${icons.arrow}</button></div></form>`:''}`;
...
function examControls(){
 ...
 const q=examQuestions[s.question];
 const helpKind=['connection','examPlot','examResistance','examCurrent','examCondition'][s.question];
 return `<div class="panel-eyebrow">QUESTION ${s.question+1} / 5</div><h2>${q.title}</h2><p class="concept-note">${q.note}</p>${s.question===0?'<p class="concept-note terminal-key">A・B は抵抗器の両側。C・D は電池の両側です。</p>':''}${s.question===2?`<div class="formula"><span>抵抗を求める</span><strong>R ＝ V ÷ I</strong></div>${symbolKey()}<p class="worked-example">前の実験では、3.0 V と 0.30 A から 3.0 ÷ 0.30 ＝ 10 Ω と求めました。</p>`:''}${s.question===3?`<div class="formula"><span>電流を求める</span><strong>I ＝ V ÷ R</strong></div>${symbolKey()}`:''}${problemHelp(helpKind)}
...
function assistPanel(){if(s.stage>=6)return '';const step=guide();return `<div class="assist-panel">${s.answerShown?`<p><b>見本・答え</b> ${step.text} ${stages[s.stage].insight}</p>`:'<button class="text-button" data-action="answer">見本・答えを見る</button>'}</div>`;}
function render({focusTitle=false}={}){
 ...
 const r=model(),st=stages[s.stage],wasDone=completed.includes(s.stage),justDone=!wasDone&&finished(),done=wasDone||justDone,step=guide(),demo=pendingDemo(),demoFinished=Boolean(demo&&demoStep===coachSteps[demo].length),demoCircuit=demoFinished&&demo==='wiring'?{...s,wires:SINGLE_WIRES,closed:true}:s,demoResult=demoFinished&&demo==='wiring'?solveCircuit(demoCircuit):r;
 if(demo!==demoSession){demoStep=firstDemoStep(demo);demoSession=demo;}
 ...
 app.className=`stage-${s.stage}${done?' stage-complete':''}${celebrationStage===s.stage?' just-achieved':''}${firstCircuitWired&&!s.closed?' needs-switch':''}${s.stage===2&&s.range?` task-target-${step.target}`:''}${s.hint?` hint-target-${step.target}`:''}`;
 ...
 <main><div class="chapter-line"><span>CHAPTER ${String(st.section+1).padStart(2,'0')}</span><span class="chapter-dash"></span><span>${st.name}</span><span class="stage-count">${String(s.stage+1).padStart(2,'0')} / 09</span></div><div class="title-row"><div><h1 tabindex="-1">${done&&isExam?'見え方が変わっても、同じ回路。':st.title}</h1><p class="subtitle">${isExam?'図の下の四角い部品が抵抗器 R。表は電圧と流れた電流の記録です。':st.intro}</p></div>${s.stage<6?`<button class="hint-button" data-action="hint" aria-expanded="${s.hint}">${icons.hint}<span>ヒント</span></button>`:''}</div>
 ${s.hint?`<div class="hint-strip" role="note">${hintContent(step)}<button data-action="hint" aria-label="ヒントを閉じる">×</button></div>`:''}
 ${isExam?'<p class="experiment-text">同じ抵抗器 R にかける電圧だけを変え、電流を調べました。</p>':''}
 <div class="workbench ${s.stage===6||isExam?'has-data':''} ${isExam?'exam':''} ${s.stage===2&&!s.tour?'lesson-demo':''}"><section class="circuit-area ${demo?'demo-auto':''} ${demoFinished?'demo-finished':''} ${has('current-meter-ok')||has('voltage-lamp-ok')?'meter-success':''}" aria-label="回路を操作する"><div class="canvas-toolbar"><span class="canvas-label"><i class="${demoResult.current>0?'on':''}"></i>${circuitStatus(demoResult)}</span><div class="view-toggle" aria-label="回路の表示"><button data-view="real" aria-pressed="${!s.schematic}" class="${!s.schematic?'active':''}">実物</button><button data-view="diagram" aria-pressed="${s.schematic}" class="${s.schematic?'active':''}">回路図</button></div></div>
 <div class="circuit-wrap ${isExam&&has('rearranged')?'rearranged':''}">${circuitView(demoCircuit,demoResult,{exam:isExam,overlay:demo&&(!demoFinished||demo!=='wiring')?coachOverlay(demo,demoFinished?demoStep-1:demoStep):''})}</div><div class="circuit-readings" aria-label="電源と回路全体の測定値"><span>電池 <b>${pretty(demoCircuit.voltage,1)}</b> V</span><span>全体 <b>${pretty(demoResult.current)}</b> A</span></div><div class="canvas-bottom"><span>${s.stage===2&&s.range?'Aを丸い「＋」へ運ぶ':s.stage===3?'測る2つの端子をタップ':s.stage===0?(firstCircuitWired&&!s.closed?'スイッチを閉じる':s.selected?'もう一方の端子をタップ':'端子 → 端子'):s.stage===5?(s.tool==='voltage'?'はかりたいものの両端を順にタップ':s.tool==='current'?'はかる場所の丸い「＋」をタップ':'右側で計器を選ぶ'):s.tool==='voltage'?'2点を選んで測定':s.stage>=6?'電圧と電流の関係を観察':s.stage===1?'電流の向き：電池の外側は＋ → −':'スイッチをタップ'}</span><div class="canvas-actions"><button data-action="undo" class="icon-button" aria-label="操作を一つ戻す" ${history.length?'':'disabled'}><svg viewBox="0 0 24 24" fill="none"><path d="m9 6-5 5 5 5M4 11h10a5 5 0 0 1 0 10"/></svg></button><button data-action="reset" class="icon-button" aria-label="このステージをやり直す">${icons.reset}</button></div></div></section>
 <aside class="control-panel" aria-label="実験の操作と問い">${demo?coachControls(demo,demoStep):controls(r)+assistPanel()}</aside>
 ${s.stage===6||isExam?graphPanel():''}</div>
 <div class="feedback ${s.feedback?'visible':''}" role="status" aria-live="polite">${escaped(s.feedback)}</div>
 <footer class="lesson-footer"><div class="lesson-insight ${done?'revealed':''}">${done?`<span class="insight-mark">${icons.check}</span><p>${st.insight}</p>`:s.stage===0?'':`<span class="quiet-label">今やること：${step.text}</span>`}</div>${s.stage<8?`<button class="primary next-button ${done?'ready':''}" data-action="next" ${done?'':'disabled'}>${done?nextLabel():'次へ'} ${icons.arrow}</button>`:`<button class="primary next-button ${done?'ready':''}" data-action="${done?'review':'back-to-experiment'}">${done?'振り返る':'実験で確かめる'} ${icons.arrow}</button>`}</footer>
...
function selectChoice(key,value){
 ...
 if(value===answers[key]){s.answers[key]=value;s.feedback={flow:'回路で確かめよう。',current:'どちらも 0.20 A。',branch:'二つのつなぎ方で、電球 Bを外してみよう。',sum:'0.40 ＋ 0.20 ＝ 0.60 A。',exam:'抵抗器を変えずに、電圧だけを変える。'}[key];if(key==='exam'){s.confirmed=true;s.helpStep=0;s.answerShown=false;}}
...
app.addEventListener('click',e=>{
 ...
 if(action==='replay-demo'){const type=s.stage===0?'wiring':s.stage===2?'current':'voltage';if(type==='wiring')wiringDemoOpen=true;else delete demoSeen[type];demoStep=0;render();document.querySelector('.circuit-area')?.scrollIntoView({block:'nearest'});return;}
 if(action==='demo-restart'){demoStep=0;render();return;}
 ...
 if(action==='home'){home=true;wiringDemoOpen=false;drawer=false;modal=null;save();render();return;}
 ...
 if(action==='about'||action==='review'||action==='reset'||action==='meter-help'){drawer=false;modal=action;render();return;}
 ...
 if(action==='problem-hint'){s.helpStep=Math.min(2,s.helpStep+1);render();return;}
 ...
 if(el.dataset.choice){selectChoice(el.dataset.choice,el.dataset.value);if(el.dataset.choice==='exam'&&!s.confirmed)s.helpStep=Math.min(2,s.helpStep+1);}
 ...
 if(el.dataset.plot){const value=Number(el.dataset.plot);s.graphGuess=value;if(s.stage===6){if(value===.4){s.answers.graph=value;s.helpStep=0;s.answerShown=false;s.feedback='4 Vなら 0.40 A。原点を通る直線になる。';}else{s.feedback='表の 2 V の行から考えよう。下にヒントを出しました。';s.helpStep=Math.min(2,s.helpStep+1);}}else if(s.stage===8){if(value===.3){s.confirmed=true;s.helpStep=0;s.answerShown=false;s.feedback='表の 3.0 V、0.30 Aと対応しています。';}else{s.feedback='表の 3.0 V の行を見よう。下にヒントを出しました。';s.helpStep=Math.min(2,s.helpStep+1);}}}
 if(action==='check-connection'){if(sameEdge(s.examConnections,['a','b'])){s.confirmed=true;s.helpStep=0;s.answerShown=false;s.feedback='抵抗器 Rの両端につながりました。';}else{s.feedback=sameEdge(s.examConnections,['p','n'])?'電池の両端もこの回路では同じ電圧です。この問いでは抵抗器の端子 A・Bへ直接つなぎます。':'図の下の抵抗器 R の左 A と右 B を見よう。下にヒントを出しました。';s.helpStep=Math.min(2,s.helpStep+1);}}
 if(action==='next-question'&&s.confirmed){s.question++;s.answerShown=false;s.helpStep=0;s.confirmed=false;s.graphGuess=null;s.feedback='';}
...
function clearDragPreview(){pointerDrag?.ghost?.remove();document.querySelector('.drag-target')?.classList.remove('drag-target');}
app.addEventListener('pointerdown',e=>{const source=e.target.closest('[data-drag-meter],[data-drag-probe]');if(!source||e.button!==0||!e.isPrimary)return;markDemoSeen(pendingDemo());const kind=source.dataset.dragMeter!==undefined?'meter':'probe';const ghost=document.createElement('div');ghost.className='drag-ghost';ghost.textContent=kind==='meter'?'A':'V';ghost.style.left=`${e.clientX+18}px`;ghost.style.top=`${e.clientY-42}px`;document.body.append(ghost);pointerDrag={kind,id:e.pointerId,ghost};source.setPointerCapture(e.pointerId);e.preventDefault();});
document.addEventListener('pointermove',e=>{if(!pointerDrag||pointerDrag.id!==e.pointerId)return;pointerDrag.ghost.style.left=`${e.clientX+18}px`;pointerDrag.ghost.style.top=`${e.clientY-42}px`;const target=document.elementFromPoint(e.clientX,e.clientY)?.closest(pointerDrag.kind==='meter'?'[data-slot]':'[data-node]');document.querySelector('.drag-target')?.classList.remove('drag-target');target?.classList.add('drag-target');});
document.addEventListener('pointerup',e=>{if(!pointerDrag||pointerDrag.id!==e.pointerId)return;const {kind}=pointerDrag;clearDragPreview();pointerDrag=null;const target=document.elementFromPoint(e.clientX,e.clientY)?.closest('[data-slot],[data-node]');skipDragClick=true;setTimeout(()=>{skipDragClick=false;},0);remember();if(kind==='meter'&&target?.dataset.slot)placeMeter(target.dataset.slot);else if(kind==='probe'&&target?.dataset.node)placeProbe(target.dataset.node);else s.feedback=kind==='meter'?'ヒント：Aを回路の丸い「＋」へ運ぼう。':'ヒント：電圧計は端子へ。';render();});
document.addEventListener('pointercancel',e=>{if(pointerDrag?.id===e.pointerId){clearDragPreview();pointerDrag=null;}});
...
app.addEventListener('submit',e=>{e.preventDefault();const form=e.target;const raw=new FormData(form).get('answer')?.toString().trim().replace(/[０-９]/g,c=>String.fromCharCode(c.charCodeAt(0)-0xfee0)).replace('．','.');const n=Number(raw);remember();if(!raw||!Number.isFinite(n)){s.feedback='数値を入れてください。';}else if(form.dataset.form==='resistance'){if(Math.abs(n-20)<1e-8){s.answers.resistance='20';s.helpStep=0;s.answerShown=false;s.feedback='3.0 ÷ 0.15 ＝ 20 Ω。';}else{s.feedback='どの数を V と I に入れるか確認しよう。下にヒントを出しました。';s.helpStep=Math.min(2,s.helpStep+1);}}else if(form.dataset.form==='exam'){const answer=examQuestions[s.question].answer;if(Math.abs(n-answer)<1e-8){s.confirmed=true;s.helpStep=0;s.answerShown=false;s.feedback=s.question===2?'1.0 ÷ 0.10 ＝ 10 Ω。':'4.0 ÷ 10 ＝ 0.40 A。';}else{s.feedback=s.question===2?'表の同じ行の電圧と電流を使おう。下にヒントを出しました。':'求めるのは電流 I。下にヒントを出しました。';s.helpStep=Math.min(2,s.helpStep+1);}}render();});
