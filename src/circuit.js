...
return `<svg class="circuit-svg ${active?'is-flowing':''} ${schematic?'is-schematic':''}" viewBox="0 0 680 440" role="group" aria-label="${schematic?'回路図':'実物風の回路'}">
 ...
 ${resistance?resistor(340,335,schematic,exam?'抵抗器 R':`${s.resistance} Ω`):bulb(340,335,r.powers.lamp1||0,schematic,false,s.topology==='single'?'':'電球 A',s.topology!=='single')}
