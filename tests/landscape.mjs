...
const results=[];
try{
for(const device of [{name:'landscape-desktop',width:1280,height:800},{name:'landscape-tablet',width:1194,height:834},{name:'landscape-mobile',width:844,height:390},{name:'landscape-small',width:667,height:375}]){
 ...
 const shot=async name=>{const layout=await page.evaluate(()=>({width:innerWidth,height:innerHeight,scrollWidth:document.documentElement.scrollWidth,scrollHeight:document.documentElement.scrollHeight,controls:[...document.querySelectorAll('.control-panel button,.circuit-readings,.lesson-footer')].filter(el=>el.getBoundingClientRect().height).map(el=>({label:el.textContent.trim().slice(0,25),bottom:el.getBoundingClientRect().bottom}))}));assert.ok(layout.scrollWidth<=layout.width,JSON.stringify(layout));assert.ok(layout.scrollHeight<=layout.height+1,JSON.stringify(layout));assert.ok(layout.controls.every(c=>c.bottom<=layout.height+1),JSON.stringify(layout));await page.screenshot({path:`artifacts/${device.name}-${name}.png`,fullPage:true});};
